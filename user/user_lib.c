...
int printu_dir(const char *s, ...)
{
	...
	// make a syscall to implement the required functionality.
	return do_user_call(SYS_user_print_dir, (uint64)buf, n, 0, 0, 0, 0, 0);
}
...
void MKDIR(char *commandList, int *index)
{
	...
	int result = mkdir_u(path);
	if (result == -1)
	{
		printu("mkdir: cannot create directory '%s': File exists\n", arg);
	}
}
...
void TOUCH(char *commandList, int *index)
{
	...
	if (fd < 0)
	{
		printu("touch: cannot create file '%s': File exists\n", arg);
	}
	else
	{
		close(fd);
