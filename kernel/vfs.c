...
struct file *vfs_opendir(const char *path)
{
	...
	{
		if (file_dentry->dentry_inode->i_ops->viop_hook_opendir(file_dentry->dentry_inode, file_dentry) != 0)
		{
...
int vfs_closedir(struct file *file)
{
	...
	{
		if (file->f_dentry->dentry_inode->i_ops->viop_hook_closedir(file->f_dentry->dentry_inode, file->f_dentry) != 0)
		{
