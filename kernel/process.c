...
/*
	modified at 2024/2/28
	原先的代码支持大页分配 但是与头哥的测试的分配方式不同 导致地址的间距过不了测试
	故而修改分配机制
...
// 第二种分配方式：使用一个free块 + 一堆u_gfree_page
static void alloc_from_free_and_g_ufree_page(uint64 va, uint64 n) {
	remove_ptr(&(current->rib_free), va); // 从free链表中本空闲块
	m_rib* pa = (m_rib*)(user_va_to_pa((pagetable_t)current->pagetable, (void*)va));
	uint64 remain = n - (pa->cap - sizeof(m_rib)); // 还有remain个字节需要分配
	uint64 new_pages = remain / PGSIZE + (remain % PGSIZE != 0); // 需要new_pages个页面
	// 分配这些页
	for (int _ = 1; _ <= new_pages; _ ++) {
		uint64 new_page_pa = (uint64)alloc_page();
		uint64 new_page_va = g_ufree_page;
		g_ufree_page += PGSIZE;
		memset((void*)new_page_pa, 0, PGSIZE); // 页面置'\0'
		user_vm_map((pagetable_t)current->pagetable, new_page_va, PGSIZE, new_page_pa, prot_to_type(PROT_WRITE | PROT_READ, 1));
	}
	uint64 last_page_use = remain % PGSIZE;
	if (last_page_use == 0);
	else {
		uint64 next_rib_addr = get_next_rib_addr(g_ufree_page - PGSIZE + last_page_use);
		if (PGSIZE - next_rib_addr > sizeof(m_rib)) {
			// 这个剩余空间还足够放下一个m_rib
			// 则需要为这个剩余空间生成一个free块并且插入到free链表中
			// 则参数中的va对应的pa处的cap修改为next_rib_addr - va;
			...
			// 参数中的va对应的pa处的cap修改为pa->cap + new_pages * PGSIZE
...
#define NEED 1
#define NOT_NEED 0
// 函数名意思:是否需要开始的地址为g_ufree_page的页面
//           is need st      is  g_ufree_page
static int is_need_st_is_g_ufree_page(uint64 n, uint64 *va_ptr)
{ // va为返回的虚拟地址，只有当为false是va有效
	...
	while (va != -1)
	{
		...
		{ // 需要减去m_rib的大小
			...
			*va_ptr = va + sizeof(m_rib);
			// 进行分配
			alloc_from_free(va, n); // NOTE:第一种分配方式
			return NOT_NEED;
		}
		va = pa->next;
		// max_va = va > max_va ? va : max_va; // NOTE:不应该放在这里。。。
	}
	...
	if (max_va + max_pa->cap == g_ufree_page)
	{
		...
		*va_ptr = max_va + sizeof(m_rib); // 设置返回的va
		// sprint("come here %lx\n", max_va + sizeof(m_rib));
		alloc_from_free_and_g_ufree_page(max_va, n); // 分配 NOTE:第二种分配方式
		return NOT_NEED;
	}
	else {
		// sprint("come to else\n");
...
{
	uint64 va;
	if (is_need_st_is_g_ufree_page(n, &va) == NEED)
	{
